...
By default, once the Payment closes, Voucherly.js sends your page to the `redirectOkUrl` or `redirectKoUrl` of the Payment. With `redirect: "if_required"` your page stays where it is and the outcome reaches `onPaymentComplete` and `onPaymentError`.

`loadVoucherly()` injects the script once; later calls share the same promise. It resolves with `null` when there is no `window`, so it is safe to call during server-side rendering. The `paymentId` comes from a Payment created on your server with your secret key: see the [integration guide](https://docs.voucherly.it/en/guides/online-payments/components/).
...
- [Voucherly Components guide](https://docs.voucherly.it/en/guides/online-payments/components/)
- [Voucherly.js reference](https://docs.voucherly.it/en/guides/online-payments/components/reference/)
